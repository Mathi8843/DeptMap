...
import { GitBranch, Eye, EyeOff, ShieldCheck, Sun, Moon, ArrowRight, Check } from "lucide-react";
import ThreeDGrid from "@/components/ui/ThreeDGrid";
import TiltCard from "@/components/ui/TiltCard";

const PERKS = [
  "Free forever for 1 repository",
...
export default function SignupClient() {
  ...
  return (
    ...
    >
      ...
      {/* Ambient glow orbs */}
      <div aria-hidden="true" className="absolute pointer-events-none" style={{ top: "-15%", right: "-10%", width: 520, height: 520, borderRadius: "50%", background: "radial-gradient(circle, rgba(99,102,241,0.20) 0%, transparent 70%)", filter: "blur(60px)" }} />
      <div aria-hidden="true" className="absolute pointer-events-none" style={{ bottom: "-20%", left: "-10%", width: 480, height: 480, borderRadius: "50%", background: "radial-gradient(circle, rgba(99,102,241,0.12) 0%, transparent 70%)", filter: "blur(50px)" }} />
      ...
      <nav className="relative z-20 flex items-center justify-between px-6 py-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <Link href="/login" className="font-mono text-[10px] uppercase tracking-[1.5px] font-bold px-4 py-2.5 border border-border-subtle text-text-sub hover:text-text-main hover:border-border-glow rounded-xl transition-all flex items-center gap-1.5">
            Sign In <ArrowRight size={11} />
          </Link>
        </div>
      </nav>

      {/* Two-column layout — left perks, right form */}
        ...
        <div className="w-full max-w-4xl flex flex-col lg:flex-row items-center gap-8 lg:gap-14">
          ...
          <div className="hidden lg:flex flex-col flex-1 space-y-5">
            <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/20 rounded-full px-3 py-1.5 w-fit">
              ...
              <span className="font-mono text-[10px] uppercase tracking-[2px] text-indigo-600 dark:text-indigo-400 font-bold">Free forever for 1 repo</span>
            </div>
            <h1 className="text-3xl font-extrabold tracking-tight leading-tight text-text-main">
              Secure your AI app<span className="block text-indigo-600 dark:text-indigo-400 mt-1">in 2 minutes.</span>
            </h1>
            <p className="text-sm text-text-sub leading-relaxed max-w-xs">
              Scan for OWASP vulnerabilities, detect slopsquatted packages, and get one-click GitHub PR fixes — in plain English.
            </p>
            ...
            <TiltCard className="glass-card rounded-3xl shadow-2xl" maxTilt={8} perspective={1000} scale={1.01} shineColor="rgba(99,102,241,0.07)">
              <div className="p-6 space-y-4">
                ...
                <div className="text-center space-y-1.5">
                  ...
                  <div>
                    <h2 className="text-lg font-extrabold text-text-main tracking-tight">Create your account</h2>
                    <p className="text-[11px] text-text-muted mt-0.5">Free forever for 1 repo. No credit card.</p>
                  </div>
                </div>
                ...
                {/* Error */}
                {errorMsg && (
                  <div id="signup-error" role="alert" aria-live="polite" className="bg-rose-500/5 border border-rose-500/15 rounded-xl p-3 text-center text-xs text-rose-500 font-medium">
                    {errorMsg}
                  </div>
                )}

                {/* Form */}
                <form onSubmit={handleEmailSignUp} className="space-y-2.5" aria-label="Create account form" noValidate>
                  <div>
                    <label htmlFor="signup-name" className="block text-xs font-semibold text-text-sub mb-1">Full Name</label>
                    <input id="signup-name" type="text" autoComplete="name" placeholder="Your Name" value={name} onChange={(e) => setName(e.target.value)} aria-invalid={!!errorMsg} aria-describedby={errorMsg ? "signup-error" : undefined} className="w-full bg-bg-deep border border-border-subtle rounded-xl px-4 py-2 text-sm text-text-main placeholder-text-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 transition-all" />
                  </div>
                  <div>
                    <label htmlFor="signup-email" className="block text-xs font-semibold text-text-sub mb-1">Email Address</label>
                    <input id="signup-email" type="email" autoComplete="email" placeholder="name@company.com" value={email} onChange={(e) => setEmail(e.target.value)} aria-invalid={!!errorMsg} aria-describedby={errorMsg ? "signup-error" : undefined} className="w-full bg-bg-deep border border-border-subtle rounded-xl px-4 py-2 text-sm text-text-main placeholder-text-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 transition-all" />
                  </div>
                  <div>
                    <label htmlFor="signup-password" className="block text-xs font-semibold text-text-sub mb-1">Password</label>
                    <div className="relative">
                      <input id="signup-password" type={showPass ? "text" : "password"} autoComplete="new-password" placeholder="Min 6 characters" value={password} onChange={(e) => setPassword(e.target.value)} aria-invalid={!!errorMsg} aria-describedby={errorMsg ? "signup-error" : "signup-pw-hint"} className="w-full bg-bg-deep border border-border-subtle rounded-xl px-4 py-2 text-sm text-text-main placeholder-text-muted/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500/40 transition-all pr-12" />
                    ...
                    <p id="signup-pw-hint" className="text-[10px] text-text-muted mt-0.5">Must be at least 6 characters</p>
                  </div>
                  <button type="submit" disabled={loading} aria-busy={loading} className="w-full py-2.5 bg-indigo-500 hover:bg-indigo-600 text-white font-mono text-[11px] uppercase tracking-[1.5px] font-bold rounded-xl transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50 shadow-lg shadow-indigo-500/20">
                    {loading ? (
                      <span className="flex items-center gap-2"><svg className="animate-spin h-4 w-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Creating account…</span>
                    ) : "Create Free Account →"}
                  </button>
                </form>
                ...
                <p className="text-center text-[11px] text-text-muted">
                  ...
                  <Link href="/login" className="text-indigo-500 hover:text-indigo-400 font-semibold transition-colors">Sign in</Link>
                </p>
              </div>
            </TiltCard>
          </div>
