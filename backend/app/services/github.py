...
def create_fix_pull_request(
    ...
    commit_message = f"security-fix: {issue_title}\n\nAuto-generated fix by Risk Guard AI AI. Issue ID: {issue_id}"
    repo.update_file(
        path=file_path,
    ...
    body_parts = [
        "## 🔒 Security Fix — Auto-generated by Risk Guard AI\n",
        f"**Issue:** {issue_title}\n",
        f"**File:** `{file_path}`\n",
        "This pull request was automatically created by Risk Guard AI. The AI-generated fix has been applied to resolve the detected vulnerability.\n",
    ]

    if plain_english_body:
        body_parts.append(f"## What changed\n{plain_english_body}\n")

    if impact_bullets:
        impact_markdown = "\n".join([f"- {b}" for b in impact_bullets])
        body_parts.append(f"## Why\n{impact_markdown}\n")

    body_parts.append(f"---\n*Issue ID: `{issue_id}`*")
    pr_body = "\n".join(body_parts)

    pr = repo.create_pull(
        title=f"[Risk Guard AI] Security Fix: {issue_title}",
        body=pr_body,
        head=branch_name,
        base=base_branch,
    )
